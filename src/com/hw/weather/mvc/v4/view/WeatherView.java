package com.hw.weather.mvc.v4.view;

import java.util.InputMismatchException;
import java.util.List;
import java.util.Scanner;

import com.hw.weather.mvc.v4.controller.WeatherController;
...
public class WeatherView {
	...
	private void findAll() {
		List<WeatherMsDto> weathers = weatherController.findAll();
		System.out.println("-------------------------------------------------------");
		if(!weathers.isEmpty()) {
			for(WeatherMsDto weather : weathers) {
				StringBuilder sb = new StringBuilder();
				sb.append("날씨 번호 : ");
				sb.append(weather.getWeatherId());
				sb.append(",\t날씨 종류 : ");
				sb.append(weather.getWeatherName());
				sb.toString();
	...
	private void findById() {
		...
		sb.append(",\t정보를 제공받은 위성이름 : ");
		sb.append(weather.getMsName());
		sb.toString();
		System.out.println(sb);
		System.out.println("-------------------------------------------------------");
	}
	...
	private void updateWeather() {
		...
		while(true) {
			...
			WeatherMsDto weather = weatherController.findById(id);
			if(weather != null) {
				break;
			} else {
